...
def upload_vector(item, index):
    id = str(item[2]['id'])
    book_id = str(item[2]['book_id'])
    vector = item[1]['data'][0]['embedding']

    index.upsert(vectors = [(id, vector)], namespace = book_id)

    # if we're here, assume it worked
    sql = "UPDATE book_content SET vector_stored = 1 WHERE id = %s"
    args = [id]
    db.query(sql, args)
...
def main(argv):
    ...
    with open(filepath) as f:
        ...
        i = 0
        for item in data:
            upload_vector(item, index)
            print("uploaded %s of %s" % (i, len(data)))
            i = i + 1
